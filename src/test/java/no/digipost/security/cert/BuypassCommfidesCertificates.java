...
public class BuypassCommfidesCertificates {
    ...
    public static Trust createTestTrust(Clock clock) {
        return Trust.merge(new DigipostTrusts(clock).buypassAndCommfidesEnterpriseCertificates(), new Trust(clock, initTrustedCerts(), initIntermediateTrust()));
    }

    public static Trust createTestTrustWithAdditionalCerts(Clock clock, X509Certificate ... additionalTrustedCerts) {
        return new Trust(clock, concat(initTrustedCerts(), Stream.of(additionalTrustedCerts)), initIntermediateTrust());
    }

    private static Stream<X509Certificate> initTrustedCerts() {
    ...
    private static Stream<X509Certificate> initIntermediateTrust() {
