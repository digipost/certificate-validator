...
class RealOCSPCertificateValidatorTest {

    private final ControllableClock clock = ControllableClock.freezedAt(LocalDateTime.of(2020, 2, 10, 12, 0).atZone(UTC));
    private final Optional<HttpHost> proxy =  Optional.ofNullable(System.getProperty("https_proxy")).map(httpsProxy -> applyUnchecked(HttpHost::create, httpsProxy));
    ...
    @Test
    void validerer_gammelt_commfides_sertifikat() {
    ...
    @Test
    void godtar_nytt_commfides_test_sertifikat() {
